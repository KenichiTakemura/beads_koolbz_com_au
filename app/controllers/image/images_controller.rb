class ImagesController < AuditsController
    ...
    begin
      if @image.thumbnailable?
        ...
        if @image.save
          ...
          @images = Image.collect_image(current_flyer, write_at)
  ...
  def destroy
    logger.debug("destroy uploaded_image")
    @image = Image.find(params[:id])
    write_at = @image.write_at
    begin
      if @image.destroy
        @images = Image.collect_image(current_flyer, write_at)
        flash[:notice] = I18n.t("upload.deleted")
      end
    rescue Exception => e
      logger.error("something wrong e => #{$!}")
      flash[:alert] = I18n.t("upload.failed_to_delete")
    end
    respond_to do |format|
      format.js
    end
